...
type prefetchConfigHelper = admissioncheck.ConfigHelper[*kueue.PrefetchRequest, kueue.PrefetchRequest]

func newPrefetchConfigHelper(c client.Client) (*prefetchConfigHelper, error) {
	return admissioncheck.NewConfigHelper[*kueue.PrefetchRequest](c)
}
...
// Option configures the prefetch controller.
...
func (c *Controller) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	// Fetch the requests
	list := &kueue.PrefetchRequestList{}
	...
	for _, prefetchReq := range list.Items {
		// Send a GET request to the URL
		resp, err := http.Get(prefetchReq.Spec.Datapath)
		if err != nil || resp.StatusCode != http.StatusOK {
			log.Error(err, "Failed to fetch datapath URL", "datapath URL", prefetchReq.Spec.Datapath)
			prefetchReq.Status.State = "incomplete"
			continue
			// return ctrl.Result{RequeueAfter: 10 * time.Second}, nil // Requeue after 10s to retry
		}

		// Read the response body
		body, err := ioutil.ReadAll(resp.Body)
		if err != nil {
			log.Error(err, "Failed to read response body", "datapath URL", prefetchReq.Spec.Datapath)
			prefetchReq.Status.State = "incomplete"
			continue
			// return ctrl.Result{}, err
		}

		log.Info("Successfully fetched datapath URL", "datapath url", prefetchReq.Spec.Datapath, "response", string(body))

		// Mark the check as complete
		prefetchReq.Status.State = "completed"
	}

	return ctrl.Result{}, nil

	// list := &kueue.PrefetchRequest{}
	// if err := c.client.List(ctx, list, client.InNamespace(wl.Namespace), client.MatchingFields{RequestsOwnedByWorkloadKey: wl.Name}); client.IgnoreNotFound(err) != nil {
	// 	return reconcile.Result{}, err
	// }
	// ownedPrs := list.Items
...
// func (c *Controller) deleteUnusedPrefetchRequests(ctx context.Context, ownedPrs []autoscaling.ProvisioningRequest, activeOrLastPRForChecks map[string]*autoscaling.ProvisioningRequest) error {
// 	log := ctrl.LoggerFrom(ctx)
...
// func (c *Controller) syncProvisionRequestsPodTemplates(ctx context.Context, wl *kueue.Workload, prName string, prc *kueue.PrefetchConfig) error {
...
// func (c *Controller) reqIsNeeded(wl *kueue.Workload, prc *kueue.PrefetchConfig) bool {
...
// 	prc, isPRC := event.Object.(*kueue.PrefetchConfig)
// 	if !isPRC {
// 		return
// 	}
// 	err := p.reconcileWorkloadsUsing(ctx, prc.Name, q)
// 	if err != nil {
// 		ctrl.LoggerFrom(ctx).V(5).Error(err, "Failure on create event", "PrefetchConfig", klog.KObj(prc))
// 	}
// }

// func (p *prcHandler) Update(ctx context.Context, event event.UpdateEvent, q workqueue.RateLimitingInterface) {
// 	oldPRC, isOldPRC := event.ObjectOld.(*kueue.PrefetchConfig)
// 	newPRC, isNewPRC := event.ObjectNew.(*kueue.PrefetchConfig)
...
// 		if err != nil {
// 			ctrl.LoggerFrom(ctx).V(5).Error(err, "Failure on update event", "PrefetchConfig", klog.KObj(oldPRC))
// 		}
// 	}
// }
...
// 	err := p.reconcileWorkloadsUsing(ctx, prc.Name, q)
// 	if err != nil {
// 		ctrl.LoggerFrom(ctx).V(5).Error(err, "Failure on delete event", "PrefetchConfig", klog.KObj(prc))
// 	}
// }
...
// 		Watches(&kueue.PrefetchRequest{}, prch).
// 		Complete(c)
// 	if err != nil {
// 		return err
// 	}
...
// 		Watches(&kueue.PrefetchConfig{}, prcACh).
// 		Complete(acReconciler)
// }
